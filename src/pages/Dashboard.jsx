        ...
        {currentlyBuilding.map((project) => (
          ...
          >
            <div className="flex items-start justify-between gap-2 min-w-0">
              ...
              <span className={`shrink-0 rounded-full px-2 py-0.5 text-[9px] sm:text-[10px] font-semibold whitespace-nowrap ${project.status === "Beta"
      ...
      <div className="flex flex-col gap-1 sm:gap-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-base sm:text-lg font-semibold text-white">
            {text}, I am <span className="text-accent-300">{user.name.split(" ")[0]}</span>
          </h1>
        </div>
      ...
      >
        ...
        >
          ...
          <motion.div variants={stagger} className="space-y-3 sm:space-y-4 w-full flex-1 min-w-0">
            ...
            <motion.div variants={stagger} className="grid gap-3 sm:gap-4 grid-cols-1 md:grid-cols-2 w-full min-w-0">
              <motion.div variants={scaleUp} className="w-full min-w-0">
                <AchievementCard />
              </motion.div>
              <motion.div variants={scaleUp} className="w-full min-w-0">
                <StreakCard />
